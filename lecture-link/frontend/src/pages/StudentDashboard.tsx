...
import { BookOpen, Download, Search, Clock, TrendingUp, FileText, ArrowRight } from 'lucide-react';
...
export function StudentDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [materialsData, coursesData] = await Promise.all([
        // Only fetch materials for this student's dept + level
        materialService.getAllMaterials({
          department: user?.department,
          level: user?.level
        }),
        // Only fetch courses for this student's dept + level
  ...
  const navigate = (path: string) => (window as any).navigate(path);
  ...
  const stats = [
    ...
    { label: 'My Courses', value: courses.length.toString(), icon: BookOpen, color: 'bg-green-500' },
  ...
  return (
    <DashboardLayout
      ...
      subtitle={`Welcome back! ${user?.department} · ${user?.level} Level`}
    >
      {/* Stats */}
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader><CardTitle className="text-lg">Quick Search</CardTitle></CardHeader>
            <CardContent>
              <div className="flex gap-4">
                <div className="flex-1 relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  <Input placeholder="Search for materials, courses..."
                    className="pl-10" value={searchQuery}
                    onChange={e => setSearchQuery(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') navigate(`/search?q=${searchQuery}`); }} />
                </div>
                <Button onClick={() => navigate(`/search?q=${searchQuery}`)} className="bg-[#0158fe] hover:bg-[#012060]">
                  Search
                </Button>
              </div>
            </CardContent>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => navigate('/resources')} className="text-[#0158fe]">
            ...
            <CardContent>
              {isLoading ? (
                <div className="flex justify-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#0158fe]" />
                </div>
              ) : recentMaterials.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  <FileText className="w-12 h-12 mx-auto mb-4 opacity-50" />
                  <p>No materials available for your department and level yet</p>
                </div>
              ) : (
        ...
        <div className="space-y-8">
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader>
              ...
              <p className="text-sm text-gray-500">{user?.department} · {user?.level} Level</p>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                <div className="flex justify-center py-8">
                ...
                <p className="text-gray-500 text-center py-4 text-sm">No courses available for your level yet</p>
              ) : (
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="space-y-3">
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate('/resources')}>
                <BookOpen className="mr-2 w-4 h-4" /> Browse Resources
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate('/search')}>
                <Search className="mr-2 w-4 h-4" /> Search Materials
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm bg-gradient-to-br from-[#0158fe] to-[#012060] text-white">
            <CardContent className="p-6">
              ...
              <p className="text-white/80 text-sm mb-4">Check out the most downloaded materials this week.</p>
              <Button variant="secondary" size="sm" onClick={() => navigate('/resources')} className="w-full">
                View Trending
              </Button>
            </CardContent>
          </Card>
